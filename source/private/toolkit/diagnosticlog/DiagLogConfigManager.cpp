//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/private/toolkit/diagnosticlog/DiagLogConfigManager.cpp $
//  
//  Owner:
//  
//  $Author: devtecbj $
...
bool16 DiagLogConfigManager::Load()
{
	...
	if (configFile.Exists())
	{
		...
		if (saxServices->ParseStream(fileStream, handler))
		{
			...
			{
				result = kFalse;
				ASSERT_FAIL(FORMAT_ARGS("Error parsing the 'Debug Settings.xml': %s", ErrorUtils::PMGetGlobalErrorString().GetPlatformString().c_str()));
			}
