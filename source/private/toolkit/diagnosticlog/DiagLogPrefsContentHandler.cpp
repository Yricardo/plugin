//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/private/toolkit/diagnosticlog/DiagLogPrefsContentHandler.cpp $
//  
//  Owner:
//  
//  $Author: devtecbj $
...
{	
	...
	else if (qname == WideString("includelist") || qname == WideString("excludelist"))
	{
		...
		ASSERT_FAIL(FORMAT_ARGS("Ignoring the unrecognized element named %s", PMString(qname).GetPlatformString().c_str()));	
...
void DiagLogPrefsContentHandler::ParseLogging(ISAXAttributes* attrs)
{
	...
	if (name == PMString("kDiagLogMinimum"))
		...
		ASSERT_FAIL(FORMAT_ARGS("Unknown logging level %s", name.GetPlatformString().c_str()));
...
void DiagLogPrefsContentHandler::ParseCategory(ISAXAttributes* attrs)
{
	...
	if (name == PMString("kDiagLogAction"))
		...
		ASSERT_FAIL(FORMAT_ARGS("Unknown category %s", name.GetPlatformString().c_str()));
