//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/private/toolkit/diagnosticlog/DiagLogIDDatabase.cpp $
//  
//  Owner: Seoras Ashby
//  
//  $Author: devtecbj $
	...
	if (fDBFile.Exists())
	{
		...
		if (saxServices->ParseStream(fileStream, handler))
		{
			if (ErrorUtils::PMGetGlobalErrorCode() != kSuccess)
				ASSERT_FAIL(FORMAT_ARGS("Error parsing the xml file: %s", ErrorUtils::PMGetGlobalErrorString().GetPlatformString().c_str()));
...
#ifdef DEBUG
	TRACE(FORMAT_ARGS("Cannot find ID %d(0x%x) in IDSpace %s!\n", id, id, GetIDSpaceName(idSpace).GetPlatformString().c_str()));
#endif
	return nil;
}
