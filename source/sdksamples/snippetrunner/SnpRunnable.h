//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/snippetrunner/SnpRunnable.h $
//  
...
class SnpRunnable
{
	public:
		...
		*/
		SnpRunnable(const CString name);
		...
		*/
		virtual const CString GetName() const {return fName;}

		/**
			Return a description of what the snippet does.
		*/
		virtual const CString	GetDescription() const {return fDescription;}

		/**
			Return dscription of the preconditions necessary for 
			successful execution of the snippet.
		*/
		virtual const CString	GetPreconditions() const {return fPreconditions;};

		/**
			Return the categories this snippet belongs to,
			formatted as a CSV string e,g, "sdk_snippet, sdk_layout".
		*/
		virtual const CString	GetCategories() const {return fCategories;}
	...
	protected:
		...
		/**	To be called by sub-classes in their constructor to describe
			...
			@param description 
		 */
		virtual void			SetDescription(const CString description) {fDescription = description;}
		/**	To be called by sub-classes in their constructor to describe
			the preconditions that must be met before SnpRunnable::CanRun will
			return kTrue.
			@param preconditions 
		 */
		virtual void			SetPreconditions(const CString preconditions) {fPreconditions = preconditions;}
		/**	To be called by sub-classes in their constructor to describe
		 ...
		 */
		virtual void			SetCategories(const CString categories) {fCategories = categories;}

	private:
		CString fName;
		CString fDescription;
		CString fPreconditions;
		CString fCategories;
};
