//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/linkwatcher/LnkWtchCacheManager.cpp $
//  
	...
	{    
		...
		if(!listData || !addedItems.empty() || !deletedItems.empty())
		{
			if (!addedItems.empty())
			{
				...
				for(int i = addedItems.Length() - 1; i >=0; --i)
					addedList.Append(addedItems[i]); 
			...
			else if (!deletedItems.empty())
			{
				...
				for(int i = deletedItems.Length() - 1; i >=0; --i)
					deletedList.Append(deletedItems[i]); 
...
void LnkWtchCacheManager::DumpUpdate(const ClassID& theChange, ISubject* theSubject, const PMIID& protocol, void* changedBy)
{
	...
	TRACEFLOW("LinkWatcher:LinkObserver"," %s, %s, %s, %s, %s, %s\n",
		...
		changedByName,
		commandStateName,
		itemListNames.GetPlatformString().c_str());
#endif // DEBUG
}
