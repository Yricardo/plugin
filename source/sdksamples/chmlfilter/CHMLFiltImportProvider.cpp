//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/chmlfilter/CHMLFiltImportProvider.cpp $
//  
...
ErrorCode CHMLFiltImportProvider::HandleDocLevelTags(IPMStream* fileStream)
{
	...
	while((mytok = mytokeniser.GetNextToken(token_value)) != kTokenEOF)
	{
		...
		case kTokenTbox:
			{
				TRACEFLOW("SDKTraceFlow","HandleDocLevelTags: Token --> kTokenTextBox %s %s>\n", 
					  TOKENTBOX.GetPlatformString().c_str(), token_value.GetPlatformString().c_str());
		...
		case kTokenIFile:
			{ 
				TRACEFLOW("SDKTraceFlow","HandleDocLevelTags: Token --> kTokenTextBox %s %s>\n", 
					  TOKENIFILE.GetPlatformString().c_str(), token_value.GetPlatformString().c_str());
			...
			case kTokenAlign:
				{
					...
					ICompositionStyle::TextAlignment textAlignment;
...
ErrorCode CHMLFiltImportProvider::LoadImage(const PMString& tokenVal)
{
	...
	do 
	{
		...
		TRACEFLOW("SDKTraceFlow","Trying to place %s", tmp->GetPlatformString().c_str());
		if(sdkFileHelper.IsExisting()) {
			...
			TRACEFLOW("SDKTraceFlow","Trying to place %s at %d , %d", tmp->GetPlatformString().c_str(), x, y);
