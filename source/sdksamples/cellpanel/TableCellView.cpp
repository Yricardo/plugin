//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/cellpanel/TableCellView.cpp $
//  
...
class TableCellView : public DVControlView
{
	...
	protected:
		/**
			Draws the bevel on the TableCellView widget.
		...
		*/
		void DrawBevel(IGraphicsPort*		gPort,
						SysRgn				updateRgn,
...
void TableCellView::Draw(IViewPort* vp, SysRgn updateRgn)
{
	do{
		...
		// DrawBevel isn't called by this sample.
		//DrawBevel(grPort, updateRgn, colors, frame);
...
/* DrawBevel
*/
void TableCellView::DrawBevel(IGraphicsPort*		grPort,
								 SysRgn				updateRgn,
							     IInterfaceColors*	colors,
							     const PMRect& 		frame	)
{
	...
	RealAGMColor highLightColor, shadowColor;
	colors->GetRealAGMColor(kInterfaceBevelHighlight, highLightColor);
	...
	grPort->moveto(border.Left()-1,		border.Bottom());
	grPort->lineto(border.Left()-1,		border.Top()-1);
	grPort->lineto(border.Right(),		border.Top()-1);
	grPort->stroke();

	// ----- Draw bevel high light along bottom right.
	grPort->setrgbcolor(highLightColor.red, highLightColor.green, highLightColor.blue);
	grPort->moveto(border.Right()+1,	border.Top());
	grPort->lineto(border.Right()+1,	border.Bottom()+1);
	grPort->lineto(border.Left(),	border.Bottom()+1);
	grPort->stroke();
