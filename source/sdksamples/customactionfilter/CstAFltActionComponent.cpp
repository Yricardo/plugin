//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/customactionfilter/CstAFltActionComponent.cpp $
//  
...
void CstAFltActionComponent::DoPerformAction(IActiveContext* ac, ActionID actionID, GSysPoint mousePoint, IPMUnknown* widget)
{
	...
	int32 numActionClasses = actionMapperPtr->GetNumMaps();
	for (int32 loop=0;  loop<numActionClasses; loop++){
		...
		for (int32 loop2=0; loop2<numStyles; loop2++){
			...
			if (actionName.Contains(styleName)){
				// found a match!
				styleUIDRef = iPDFStyleListMgr->GetNthStyleRef(loop);
				break;
			}
		}
