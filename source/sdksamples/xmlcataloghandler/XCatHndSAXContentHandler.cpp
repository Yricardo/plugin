//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/sdksamples/xmlcataloghandler/XCatHndSAXContentHandler.cpp $
//  
//  Owner: Ian Paterson
//  
//  $Author: gmatthew $
...
void XCatHndSAXContentHandler::beginGroup(IXMLParsingContext* ctx, IXCatHndParsingContext* xcatHndCtx, ISAXAttributes* attrs)
{
	...
	do {
		...
		if(got1) {
			TRACEFLOW(kXCatHndPluginName, "got %s\n", PMString(base).GetPlatformString().c_str());
		}	

		if(got2) {
			TRACEFLOW(kXCatHndPluginName, "got %s\n", PMString(qBase).GetPlatformString().c_str());
		}
		if(!(got1 || got2)) {
			break;
		}
