//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/PMPoint.h $
//  
...
class PMPoint
{
	public:
		...
		/**
		 Copy constructor. Initializes this point to the same values as the 
		 source point.
		 
		 @param aPoint		IN The point to copy
		 */
		PMPoint(const PMPoint& aPoint) : x(aPoint.x), y(aPoint.y)
			{}
		 ...
		 */
		PMPoint(const PMReal& x_orig, const PMReal& y_orig) : x(x_orig), y(y_orig)
			{}
		 ...
		 */
		PMPoint(const SysPoint& p) : x(double(SysPointH(p))), y(double(SysPointV(p)))
			{}
		 ...
		 */
		friend bool16 operator ==(const PMPoint& a, const PMPoint& b);
		...
		*/
		bool operator<( const PMPoint& rhs) const
		{
			return ToDouble(x) == ToDouble(rhs.x)
				? ToDouble(y) < ToDouble(rhs.y)
				: ToDouble(x) < ToDouble(rhs.x);
		}
...
extern PUBLIC_DECL const PMPoint kZeroPoint;
extern PUBLIC_DECL const PMPoint kInvalidPoint;
...
inline bool16 operator ==(const PMPoint& a, const PMPoint& b)
{
