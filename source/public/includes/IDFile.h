//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/IDFile.h $
//  
...
public:	
	 ...
	 */
	void swap(IDFile& rhs)
	{
		using std::swap;
		swap(static_cast<AFile &>(*this), static_cast<AFile &>(rhs));
#ifdef DEBUG
		swap(fDebugPath, rhs.fDebugPath);
#endif
#ifdef WINDOWS
		swap(fTmpPathStr, rhs.fTmpPathStr);
#endif
	...
	IDFile(IDFile&& rhs)
		...
		: AFile(std::move(rhs))
#else
        : AFile(static_cast<AFile &&>(rhs))
#endif
#ifdef DEBUG
		, fDebugPath(rhs.fDebugPath)
#endif
#ifdef WINDOWS
		, fTmpPathStr(rhs.fTmpPathStr)
#endif
	{
#ifdef DEBUG
		rhs.fDebugPath = 0;
...
{
	template <>
	inline void swap<IDFile>(IDFile &a, IDFile &b)
	{
		a.swap(b);
	}
}
	
inline IDFile& IDFile::operator=(IDFile&& rhs)
{
	using std::swap;
	swap(*this, rhs);
