//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/K2Vector.h $
//  
...
class K2Vector : public K2Internals::K2VectorBase<T, Allocator> {
	...
	public:
		...
		*/
		...
		/**
			Constructs a vector with the specified initial capacity.
			Note: the std::vector constructor allocates AND initializes the
			memory for the specified number of elements, where this one will only
			allocate the required space.
			@param capacity [IN] - the initial capacity of the vector.
		*/
		explicit 	K2Vector(typename Allocator::size_type cap) :  Inherited(static_cast<typename Inherited::size_type>(cap)) {}
		...
		*/
					K2Vector(const K2Vector& rhs) : Inherited(rhs) {}
...
class K2Vector<U*, K2Allocator<U*> > : public K2Internals::K2PtrVectorBase<U*> {
	...
	public:
		...
		explicit 	K2Vector(typename Allocator::size_type cap) :  Inherited(cap) {}
