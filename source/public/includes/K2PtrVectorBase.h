//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/K2PtrVectorBase.h $
//  
//  Owner: Mat Marcus
//  
//  $Author: gmatthew $
...
public:
	...
	explicit 	K2PtrVectorBase(size_type cap) : fArray((Inherited::size_type)cap) {}
				...
				K2PtrVectorBase(const_iterator first, const_iterator last/*, const Allocator& a = Allocator()*/)
			 	...
			 	K2PtrVectorBase(const K2PtrVectorBase& rhs) : fArray(rhs.fArray) {}
...
public:
	...
	void     	swap(K2PtrVectorBase& rhs) 				{fArray.swap(rhs.fArray);}
...
#ifdef ID_DEPRECATED
		// deprecated methods
		int32 		Length() const						{return fArray.Length();}

		// hkhalfal Aprl 18 2001
		// fArray is a vector of void* elements. If we want smart comparison of raw interface
		// pointers to be activated we should have an implementation of Location that directly
		// calls K2Find.
		int32 		Location(const T& x) const;//			{return fArray.Location(x);}
...
#ifdef ID_DEPRECATED
	template <class T>
	int32 K2PtrVectorBase<T>::Location(const T& value) const
	{
...
inline void swap(K2PtrVectorBase<T/*, Allocator*/>& x, K2PtrVectorBase<T/*, Allocator*/>& y)
{
	x.swap(y);
}
}	// namespace K2Internals
