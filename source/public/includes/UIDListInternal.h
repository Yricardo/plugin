//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/UIDListInternal.h $
//  
//  Owner: Robin_Briggs 
//  
//  $Author: gmatthew $
...
private:
		/** Constructor */
		UIDListInternal(IDataBase *db) :fRefCount(1), fArray(kUIDDefaultChunkSize), fDB(db)
		{ ; }
		...
		UIDListInternal(const UIDListInternal& copy) :fRefCount(1), fArray(copy.fArray), fDB(copy.fDB)
			{;}
		/** Constructor */
		UIDListInternal(IDataBase *db, const UID singleEntry) : fRefCount(1), fArray(1), fDB(db)
			{ fArray.push_back(singleEntry); ;}
		/** Constructor */
		UIDListInternal(const IPMUnknown *obj) : fRefCount(1), fArray(1), fDB(::GetDataBase(obj)) 
			{ fArray.push_back(::GetUID(obj)); ;}
		/** Constructor */
		UIDListInternal(const UIDRef& obj) : fRefCount(1), fArray(1), fDB(obj.GetDataBase())
