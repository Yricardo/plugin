//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/includes/PMMatrix.h $
//  
//  Owner: Jeff Argast
//  
//  $Author: devtecbj $
...
//-------------------------------------------------------------------------------------
		...
		*/
		...
		/**	Contruct a matrix with a, b, c, d, e, f as initial values:
			...
			[ e f 1]
		*/
 		PUBLIC_DECL PMMatrix( PMReal a, PMReal b, PMReal c, PMReal d, PMReal e, PMReal f);

		/**	Construct a matrix with the same perceptible value as m.
		*/
		PUBLIC_DECL PMMatrix( const PMMatrix &m);
		...
		*/
		PUBLIC_DECL PMMatrix& operator=(const PMMatrix &m);
...
//			kFactorOrder_TRKS,
		...
		*/
		...
		/**	@returns the x translation amount that would be returned in the sixth 
			parameter to GetTransformValues with TransformFactorOrder == kFactorOrder_SKRT.
		*/
		PUBLIC_DECL PMReal GetXTranslation() const;
		
		/**	@returns the y translation amount that would be returned in the seventh 
			parameter to GetTransformValues with TransformFactorOrder == kFactorOrder_SKRT.
		*/
		PUBLIC_DECL PMReal GetYTranslation() const;
	...
	private:
		friend struct PMMatrixInternal;
#if DEBUG
		mutable union { char buf[80];} private_data;
#else
		mutable union { long double d; void* p; uint64 i; char buf[80];} private_data; //temporary workaround for CS3 to protect against clients who memcpy a PMMatrix rather than call its copy constructor
#endif
		PUBLIC_DECL void SanityCheck_() const;

#if DEBUG
	public:
		mutable PMMatrixInternal* rep;
		struct { double xscale, yscale, skew, rotation, tx, ty;} debugger_display;
#endif
 };
