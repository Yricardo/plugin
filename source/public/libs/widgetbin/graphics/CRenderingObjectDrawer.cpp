//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/widgetbin/graphics/CRenderingObjectDrawer.cpp $
//  
//  Owner: SusanCL
//  
//  $Author: devtecbj $
...
void CRenderingObjectDrawer::DrawRenderObjectUI ( IViewPort* viewPort, IControlView* controlView, SysRgn updateRgn, PMRect frame ) const
{
	...
	if (listBoxAttr && (borderWidth = listBoxAttr->GetBorderWidth()) == 0)
		bInsetHilite = kFalse;
	...
	DrawRenderObjectUISwatch(gPort, swatchRt);
...
void CRenderingObjectDrawer::DrawBackground(IGraphicsPort* gPort, const IControlView* widget, const PMRect& frame, const bool16& isHilited, const bool16& bInsetHilite) const
{
	...
	if (bInsetHilite)
	{
		PMRect insetFrame(frame.Left(), frame.Top(), frame.Right() - 1, frame.Bottom() - 1);
...
void CRenderingObjectDrawer::DrawNoEditIcon(IViewPort* viewPort, const SysRect& bbox) const
{
	...
	iconRect.Right() -= (2*iconRect.Height() + kIconInset - 1); // The new icon SP_NoEdit_Xs_N.png (After icon integration) has only 1 empty pixel on the left (unlike 2 in the previous icon P_LockPencil_Sm_N.png), hence making change of 1 pixel to adjust this.
