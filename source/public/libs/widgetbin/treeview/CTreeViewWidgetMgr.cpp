//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/widgetbin/treeview/CTreeViewWidgetMgr.cpp $
//  
//  Owner: Matt Joss
//  
//  $Author: devtecbj $
...
void CTreeViewWidgetMgr::InternalSetStyle(int32 styleIdentifier)
{
	...
	if (cur != fStyleWidgetTable.end())
	{
		...
		if (widget)
		{
			...
			// Resize the panel so that it constrain dimensions to the new size
			InterfacePtr<IWidgetParent> forTreeParent(this, UseDefaultIID());
			InterfacePtr<IWindow> parentWindow((IWindow*) forTreeParent->QueryParentFor(IID_IWINDOW));
			if (parentWindow)
			{
				InterfacePtr<IControlView> windowView(parentWindow, UseDefaultIID());
				PMRect frame = windowView->GetFrame();
				// Well, I'm not sure why we need to resize the window view here, but i'm not changing this as this 
				// code has been there since a long time.
				// In case of windows hi dpi, when we get the window frame, it gives us the frame in global coordinates.
				// When we call Resize() with the dimensions that we got by calling GetFrame(), we end up making the 
				// panel bigger. So, bring the dimensions in the logical space and then pass it on.
				PMPoint newDim(frame.Width(), frame.Height());
#ifdef WINDOWS
				InterfacePtr<IControlView> treeView(this, UseDefaultIID());
				float scaleFactor = ::ToFloat(ID::os_utils::GetViewScaleFactorReal(treeView));
				if (scaleFactor != 1.0f && Utils<IWidgetUtils>()->IsWidgetInPalette(treeView))
				{
					PMMatrix scaleMatrix;
					scaleMatrix.Scale(1/scaleFactor, 1/scaleFactor);
					scaleMatrix.Transform(&newDim);
				}
#endif
				windowView->Resize(windowView->ConstrainDimensions(newDim));
			}
			else
			{
				// OWL case - there is no IWindow so this is a pretty lame solution to the problem...
			}
		}
	}
