//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/widgetbin/pageitems/basicinterfaces/CHandleShape.cpp $
//  
//  Owner:
//  
//  $Author: devtecbj $
...
{
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		InterfacePtr<IAdornmentIterator> pIt(fAdornmentList->CreateIterator(drawOrder));
		...
		while(pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			...
			pShape->DrawAdornmentHandleShape (this, drawOrder, gd, concreteSelection, flags);
		}
	}
}
	...
	)
{
	...
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		...
		while(pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			//Get the instance of the adornment
			InterfacePtr<IAdornmentHandleShape> pShape(punkAdorn, IID_IADORNMENTHANDLESHAPE);
			pShape->DrawControlPointPath (this, drawOrder, gd, flags);
		}
	}
}
	...
	)
{
	...
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		...
		while(pIt && !isHit && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			//Get the instance of the adornment
			InterfacePtr<IAdornmentHandleShape> pShape(punkAdorn, IID_IADORNMENTHANDLESHAPE);
			isHit = pShape->HitTest (this, drawOrder, gd, r, handleIndex, bDragging);
		}
	}
	...
	)
{
	...
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		...
		while(pIt && !isHit && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			//Get the instance of the adornment
			InterfacePtr<IAdornmentHandleShape> pShape(punkAdorn, IID_IADORNMENTHANDLESHAPE);
			isHit = pShape->HitTestControlPointPath (this, drawOrder, gd, r);
		}
	}
	...
	)
{
	...
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		...
		while(pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			//Get the instance of the adornment
			InterfacePtr<IAdornmentHandleShape> pShape(punkAdorn, IID_IADORNMENTHANDLESHAPE);
			const PMRect& bBox = pShape->GetPaintedAdornmentHandleBounds (this, drawOrder, itemBounds, pbtoview);

			if ( !bBox.IsEmpty() )
			{
				handleShapeBBox->Union (bBox);
			}
		}
	}
}
	...
	)
{
	...
	if ( fAdornmentList && fAdornmentList->GetNumberOfHandleAdornments() > 0 )
	{
		...
		while(pIt && !iRetTracker && ((punkAdorn = pIt->GetNextAdornment()) != nil) )
		{
			//Get the instance of the adornment
			InterfacePtr<IAdornmentHandleShape> pShape(punkAdorn, IID_IADORNMENTHANDLESHAPE);
			iRetTracker = pShape->QueryTracker (this, drawOrder, layView, where, selectionMode, index);
		}
	}
