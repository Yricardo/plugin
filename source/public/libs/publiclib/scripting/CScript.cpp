//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/publiclib/scripting/CScript.cpp $
//  
...
const ObjectScriptElement* CScript::GetObjectInfo( const RequestContext& context ) const
{
	...
	if ( scriptInfoMgr )
	{
		...
		ASSERT_MSG( pObjSI, FORMAT_ARGS( "CScript::GetObjectInfo - Object type '%s' not found in DOM %s",
			Utils<IScriptUtils>()->GetScriptID( this->GetObjectType( context ) ).GetPlatformString().c_str(), context.WhoAmI() ) ) ;
	}
	return pObjSI ;
}
