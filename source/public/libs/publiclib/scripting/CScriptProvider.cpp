//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/publiclib/scripting/CScriptProvider.cpp $
//  
...
void CScriptProvider::HandleMethodOnObjects( ScriptID methodID, IScriptRequestData* data, const ScriptList& scriptList )
{
	...
	for ( int32 i = 0 ; i < scriptList.size() ; i++ )
	{
		...
		if ( err != kSuccess )
		{
			ASSERT_MSG( returnDataList.size() == preReturnDataLen, FORMAT_ARGS( "HandleMethod request for method '%s' returned an error AND a value", requestDescription.GetPlatformString().c_str() ) ) ;
		...
		else
		{
			...
			{
				ASSERT_MSG( returnDataList.size() == preReturnDataLen, FORMAT_ARGS( "HandleMethod request for method '%s' declared as void but returned a value", requestDescription.GetPlatformString().c_str() ) ) ;
				data->AppendReturnData( scriptList[i], methodID, kSuccess ) ;	//push the script provider's success case here
			}
			else
			{
				ASSERT_MSG( returnDataList.size() > preReturnDataLen, FORMAT_ARGS( "HandleMethod request for method '%s' returned kSuccess but no value", requestDescription.GetPlatformString().c_str() ) ) ;
				ASSERT_MSG( returnDataList.size() < preReturnDataLen + 2, FORMAT_ARGS( "HandleMethod request for method '%s' returned more than one value", requestDescription.GetPlatformString().c_str() ) ) ;
#ifdef DEBUG
				if ( returnDataList.size() > preReturnDataLen )
					ASSERT_MSG( returnDataList[preReturnDataLen].GetRequestID() == methodID, FORMAT_ARGS( "HandleMethod request for method '%s' returned value for method '%s'",
						requestDescription.GetPlatformString().c_str(), Utils<IScriptUtils>()->GetScriptID( returnDataList[preReturnDataLen].GetRequestID() ).GetPlatformString().c_str() ) ) ;	//sanity check
#endif
			}
		}
		...
		if ( data->GetNumReturnData( scriptList[i] ) > preReturnDataLen )
		{
			...
			TRACEFLOW( "Script:ScriptRequestHandler", "  %s handled method '%s' with return data %s\n",
				GetIDName( ::GetClass( this ) ), requestDescription.GetPlatformString().c_str(), 
				GetAsString( returnData, data->GetRequestContext() ).GetPlatformString().c_str() ) ;
		}
...
void CScriptProvider::AccessPropertyOnObjects( ScriptID propID, IScriptRequestData* data, const ScriptList& scriptList )
{
	...
	for ( int32 i = 0 ; i < scriptList.size() ; i++ )
	{
		...
		if ( err != kSuccess )
		{
			ASSERT_MSG( returnDataList.size() == preReturnDataLen, FORMAT_ARGS( "AccessProperty request for property '%s' returned an error AND a value", requestDescription.GetPlatformString().c_str() ) ) ;
		...
		else //err == kSuccess
		{
			...
			{
				ASSERT_MSG( returnDataList.size() > preReturnDataLen, FORMAT_ARGS( "AccessProperty request to get property '%s' returned kSuccess but no value", requestDescription.GetPlatformString().c_str() ) ) ;
				ASSERT_MSG( returnDataList.size() < preReturnDataLen + 2, FORMAT_ARGS( "AccessProperty request to get property '%s' returned more than one value", requestDescription.GetPlatformString().c_str() ) ) ;
#ifdef DEBUG
				if ( returnDataList.size() > preReturnDataLen )
			...
			else
			{
				...
				ASSERT_MSG( returnDataList.size() == preReturnDataLen, FORMAT_ARGS( "AccessProperty request to put property '%s' returned a value", requestDescription.GetPlatformString().c_str() ) ) ;
		...
		if ( data->GetNumReturnData( scriptList[i] ) > preReturnDataLen )
		{
			...
			TRACEFLOW( "Script:ScriptRequestHandler", "  %s handled %s property '%s' with return data %s\n",
				GetIDName( ::GetClass( this ) ), data->IsPropertyGet() ? "get" : "set", requestDescription.GetPlatformString().c_str(), 
				GetAsString( returnData, data->GetRequestContext() ).GetPlatformString().c_str() ) ;
		}
		if ( err != kSuccess && data->GetErrorPolicy() == kReturnFirstError )
			break ;
...
#ifdef DEBUG
	InterfacePtr<IScriptInfoManager> scriptInfoMgr( Utils<IScriptUtils>()->QueryScriptInfoManager( data->GetRequestContext() ) ) ;
	ASSERT_FAIL( FORMAT_ARGS( "%s failed to handle property '%s'", GetIDName( ::GetClass( this ) ), scriptInfoMgr->FindPropertyName(propID).GetPlatformString().c_str() ) ) ;
#endif
	return kRequestNotHandledError ;
}
...
ErrorCode RepresentScriptProvider::GetObjectAny( IScriptRequestData* data, IScript* parent )
{
	...
	if ( count > 0 )
	{
		int32 index = count * ((double)rand() / (uint32)(RAND_MAX + 1));
