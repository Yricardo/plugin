//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/publiclib/commands/Command.cpp $
//  
//  Owner: Robin_Briggs
//  
//  $Author: gmatthew $
...
void Command::LogCommand( Action act )
{
	...
	{
		tl_CommandLog.Get()->WriteLog( kTrue, "%s command %s (%s) this=0x%.8x uid[0]=%.4d %s", action, name.GetPlatformString().c_str(),
							   DebugClassUtils::GetIDName( &buf,cmdClass ), this, itemUid.Get(), undoability ) ;
	}

	TRACEFLOW(kInterfaceSnapshotTrace,"%s command %s (%s) this=0x%.8x uid[0]=%.4d %s\n", action, name.GetPlatformString().c_str(),
							   DebugClassUtils::GetIDName( &buf,cmdClass ), this, itemUid.Get(), undoability );
