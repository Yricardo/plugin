//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/libs/publiclib/pageitems/basicinterfaces/CShape.cpp $
//  
//  Owner: Jack Kirstein
//  
//  $Author: devtecbj $
								   ...
								   IAdornmentShape::AdornmentDrawOrder paintOrderMask)
{
	...
	if( fAdornmentList )
	{
		InterfacePtr<IAdornmentIterator> pIt(fAdornmentList->CreateIterator(paintOrderMask));
		...
		while( pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil))
		{
			...
			if (!(optional && !optional->GetIsActive(this, gd, flags)))
			{
				pAdorn->DrawAdornment(this, paintOrderMask, gd, flags);
#ifdef DEBUG
...
void CShape::UnionPageItemAdornmentPaintedBBox(PMRect &paintBounds, const PMMatrix& innertoview)
{
	...
	if( fAdornmentList )
	{
		InterfacePtr<IAdornmentIterator> pIt(fAdornmentList->CreateIterator(IAdornmentShape::kAllAdornmentFlags, kTrue));
		IPMUnknown* punkAdorn = nil;
		while( pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil))
		{
			InterfacePtr<IAdornmentShape> pAdorn(punkAdorn, UseDefaultIID());
			PMRect aChildRect = pAdorn->GetPaintedAdornmentBounds(this, IAdornmentShape::kAllAdornmentFlags, itemBounds, innertoview);
...
void CShape::UnionPrintingPageItemAdornmentPaintedBBox(PMRect &printBounds, const PMMatrix& xform)
{
	...
	if( fAdornmentList )
	{
		...
		while( pIt && ((punkAdorn = pIt->GetNextAdornment()) != nil))
		{
			...
			if (pAdorn->WillPrint())
			{
				PMRect aChildRect = pAdorn->GetPrintedAdornmentBounds(this, IAdornmentShape::kAllAdornmentFlags, itemBounds, xform);
	...
	)
{
	if( fAdornmentList )
	{
		...
		{
			InterfacePtr<IAdornmentShape> pAdorn(punkAdorn, UseDefaultIID());
			pAdorn->Inval(this, drawOrder, gd, reasonForInval, flags);
		}
	}
}
