//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/public/interfaces/architecture/ILinkFacade.h $
//  
...
namespace Facade
{
	...
	public:
		...
		/**
		 Determines if a link's resource can be revealed in Adobe Mini Bridge. 
		 @param linkRef [IN] UIDRef of the link whose resource to check.
		 @return True if the link's resource can be revealed in Mini Bridge, else false.
		 */
		virtual bool CanRevealLinkInMiniBridge(const UIDRef& linkRef) const = 0;
		
		/**
		 Reveale a link's resource in Adobe Mini Bridge. 
		 @param linkRef [IN] UIDRef of the link whose resource to reveal in Adobe Mini Bridge.
		 @return kSuccess if the link's resource was revealed in Mini Bridge, else an error code.
		 */
		virtual ErrorCode RevealLinkInMiniBridge(const UIDRef& linkRef) const = 0;
