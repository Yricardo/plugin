//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/conditionaltextui/ConditionTagEyeballObserver.cpp $
//  
...
void ConditionTagEyeballObserver::Update(const ClassID& theChange, ISubject* theSubject, const PMIID& protocol, void* changedBy)
{
	if (theChange == kTrueStateMessage || theChange == kFalseStateMessage)
	{
		...
		if (data)
		{
			...
			InterfacePtr<IWidgetParent> treeNodeWidget(eyeballWidget->GetParent(), UseDefaultIID());

			InterfacePtr<ITreeNodeIDData> nodeData(treeNodeWidget, UseDefaultIID());
