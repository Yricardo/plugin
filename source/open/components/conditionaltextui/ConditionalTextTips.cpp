//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/conditionaltextui/ConditionalTextTips.cpp $
//  
//  Owner: Wai Cheuk
//  
//  $Author: gmatthew $
...
{ 
	...
	if (!bPastEndOfLine)
	{
		...
		int32    nOwnedItems = ownedItems.Length();
		for (int32 i = 0; ((i < nOwnedItems)); i++)
		{
			if (ownedItems.Length() > 0 && ownedItems[i].fClassID == kHiddenTextBoss)
			{
				// It is a hidden text boss.
				UID    ithOwnedItemUID = ownedItems[i].fUID;
				InterfacePtr<ITextStoryThread> storyThread(::GetDataBase(textModel), ithOwnedItemUID, UseDefaultIID());
				return GetTipContent(storyThread);
			}
		}
	}
