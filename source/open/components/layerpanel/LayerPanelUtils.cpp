//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/layerpanel/LayerPanelUtils.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: devtecbj $
	...
	if (panelMgr->IsPanelWithWidgetIDShown(kLayerParentWidgetId))
	{
		...
		if (panelControlView)
		{
			...
			if (treeController)
			{
				...
				{
					TreeNodePtr<const LayerTreeUIDNodeID>	layerNode(selectedItems[i]);
...
UIDList LayerPanelUtils::GetPageItemsSelectedInPanel(const IPMUnknown* somePanelWidget, IDataBase* frontDocDB)
{
	...
	if (treeController)
	{
		...
		for(int32 i = selectedItems.Length() - 1; i >=0; --i)
		{
			TreeNodePtr<const LayerTreeUIDNodeID>	piNode(selectedItems[i]);
