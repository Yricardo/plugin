//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/layerpanel/LayerPanelTreeViewWidgetMgr.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: devtecbj $
//  
//  $DateTime: 2015/05/09 23:36:25 $
//  
//  $Revision: #2 $
//  
//  $Change: 911923 $
...
public:
	...
	#pragma unused(node)
		...
		return rowSizeBool->GetBool() ? kLayerSmallRowHeight : kLayerRegularRowHeight;
	}
...
{	
	...
	if (LocaleSetting::GetLocale().IsProductFS(kInCopyProductFS))
	{
		...
		if (LocaleSetting::GetLocale().IsRightToLeftUI())
		{
			...
			expanderView->MoveTo(PMPoint(swatchView->GetFrame().Left() - 1 - expanderView->GetFrame().Width(),expanderView->GetFrame().Top()));

			nameWidget->MoveTo(PMPoint(expanderView->GetFrame().Left() - 1 - nameWidget->GetFrame().Width(),nameWidget->GetFrame().Top()));
		}
		else
		{
			swatchView->MoveTo(PMPoint(pencil->GetFrame().Left(),swatchView->GetFrame().Top()));

			expanderView->MoveTo(PMPoint(swatchView->GetFrame().Right() + 1,expanderView->GetFrame().Top()));

			nameWidget->MoveTo(PMPoint(expanderView->GetFrame().Right() + 1,nameWidget->GetFrame().Top()));
		}
	}
	else
	{
...
{	
	...
	if (LocaleSetting::GetLocale().IsRightToLeftUI())
		widgetFrame.Right() = widget->GetFrame().Width() - kPageItemNamePanelXLoc - (10 * (indentLevel - 1));
	else
		widgetFrame.Left() = kPageItemNamePanelXLoc + (10 * (indentLevel - 1));
...
{	
	...
	PMReal h = bSmallRows ? kLayerSmallRowHeight : kLayerRegularRowHeight;
...
//	TRACEFLOW("Layer","LayerPanelTreeViewWidgetMgr::CreateWidgetForNode\n");
	...
	/* Breaking the CreateObject() call into CreateObjectNoInit() and DoPostCreate(),
	 * since to set up the IWindow on the Panel before the panel is created. The panel 
	 * creation needs a theme font and that font is available at the window level
	 */
	//IControlView* view =  (IControlView*) ::CreateObject(::GetDataBase(this),RsrcSpec(LocaleSetting::GetLocale(), kLayerPanelPluginID, kViewRsrcType, layerNode->IsLayerEntry() ? kLayerElementPanelRsrcID : kLayerPageItemElementPanelRsrcID),IID_ICONTROLVIEW);
	IPMUnknown *returnVal = ::CreateObjectNoInit (::GetDataBase(this),RsrcSpec(LocaleSetting::GetLocale(), kLayerPanelPluginID, kViewRsrcType, layerNode->IsLayerEntry() ? kLayerElementPanelRsrcID : kLayerPageItemElementPanelRsrcID),IID_ICONTROLVIEW);
