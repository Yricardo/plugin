//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/spellpanel/SpellPanelOptions.cpp $
//  
//  Owner: 
//  
//  $Author: gmatthew $
...
bool16 SpellPanelOptions::AddToIgnoreList( const PMString &rIgnore ) 
{
	...
	if (!isCommand)
	{
		...
		if (!fMemoryIgnoredWordsDictionary)
		{
			...
			fMemoryIgnoredWordsDictionary = pDictService->CreateNewUserDict(dummyFileName, IUserDictService::kCreateMemoryOnly);
		}
		WideString wideIgnoreWord(rIgnore);
		pDictService->AddToUserDict(fMemoryIgnoredWordsDictionary, wideIgnoreWord, nil, kTrue);
	}
...
bool16 SpellPanelOptions::RemoveFromIgnoreList( const PMString &rIgnore ) 
{
	...
	if (!isCommand)
	{
		...
		WideString wideIgnoreWord(rIgnore);
		pDictService->RemoveFromUserDict(fMemoryIgnoredWordsDictionary, wideIgnoreWord);
	}
...
bool16 SpellPanelOptions::ClearIgnoreList()
{
	...
	if (fMemoryIgnoredWordsDictionary)
	{
		...
		InterfacePtr<IUserDictService> pDictService(pLinguMgr->QueryUserDictService( pAllLanguage ));
		pDictService->ClearUserDict(fMemoryIgnoredWordsDictionary);
	}
