//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/buttonui/actiondatapanels/showhide/ShowHideFieldsPanelObserver.cpp $
//  
...
{	
	...
	if (document)
	{
		...
		for(int32 j = 0; j < iSpread->GetNumPages(); j++)
		{
			...
			{
				PMString name =  GetFieldName(db,forms[i]);

				// Check that the name is not empty and don't insert duplicate names.
				if (!name.IsEmpty() && formNames.find(name) == formNames.end())
				{
					fields.push_back(K2Pair<UID, IShowHideFieldActionData::Action>(forms[i], IShowHideFieldActionData::kNone));
