//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/linksui/LinksUIPanelPrefs.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: gmatthew $
...
LinksUIPanelPrefs::LinksUIPanelPrefs(IPMUnknown * boss)
	...
	fSortAscending(kTrue),fCurrentRowHeight(kDefaultLinksRowHeight),
	fCurrentDocDB(nil),fAutoSizeLinkInfo(kFalse),fShowThumbnail(kTrue),fShowInfoThumbnail(false),fCollapseMultiple(kTrue)
{
	...
	for (int i = numHandlers - 1; i >= 0; --i) 
	{
		...
		if(infoProvider && std::find(orderedInfoClasses.begin(),orderedInfoClasses.end(),::GetClass(infoProvider)) == orderedInfoClasses.end())
		{
			...
			TRACEFLOW("Links:LinksUI","   %s\n",debugProviderName.GetPlatformString().c_str());
#endif
			fAllInfoProviders.push_back(infoProvider);
...
{
	return fAllInfoProviders.Length();
}

ILinkInfoProvider* LinksUIPanelPrefs::QueryNthInfoProvider(int32 nth) const
{
	ASSERT_MSG(nth >= 0 && nth < fAllInfoProviders.Length(),"LinksUIPanelPrefs::QueryNthInfoProvider asked for out of range provider");
...
{
	ASSERT_MSG(nth >= 0 && nth < fCurrentColumnWidths.Length(),"LinksUIPanelPrefs::GetNthColumnWidth asked for out of columnWidth");
	return fCurrentColumnWidths[nth];
}

void LinksUIPanelPrefs::SetColumnWidths(const K2Vector<int32>& columnWidths)
{
	ASSERT_MSG(columnWidths.Length() == fCurrentColumnWidths.Length(),"SetColumnWidths specifying wrong number of widths!");
...
{
	return fCurrentColumnsInfoProviders.Length();
}

ILinkInfoProvider* LinksUIPanelPrefs::QueryNthShowingInfoColumn(int32 nth) const
{
	ASSERT_MSG(nth >= 0 && nth < fCurrentColumnsInfoProviders.Length(),FORMAT_ARGS("LinksUIPanelPrefs::QueryNthShowingInfoColumns asked for out of range provider(%d)",nth));
...
{
	return fCurrentMoreInfoProviders.Length();
}

ILinkInfoProvider* LinksUIPanelPrefs::QueryNthMoreInfoProvider(int32 nth) const
{
	ASSERT_MSG(nth >= 0 && nth < fCurrentMoreInfoProviders.Length(),"LinksUIPanelPrefs::QueryNthMoreInfoColumns asked for out of range provider");
