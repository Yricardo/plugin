//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/linksui/LinksUIUtils.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: gmatthew $
...
void LinksUIUtilsImpl::GetEditOriginalEditorList(AppInfoList& appList, UIDList &resources)
{
	...
	for (int32 resourceIndex = 1; appList.size() > 0 && resourceIndex < resources.Length(); resourceIndex++)
	{
		...
		for (int32 editorIndex = appList.size() - 1; editorIndex >= 0; editorIndex--)
		{
			...
			for (int32 compareIndex = 0; !bFound && compareIndex < compareAppList.Length(); compareIndex++)
			{
				if (appList[editorIndex] == compareAppList[compareIndex])
				{
					bFound = kTrue;
				}
			}
...
bool LinksUIUtilsImpl::IsMenuActionEntirelyOnLinkResourceRows(IPMUnknown* widget)
{
	...
	if(treeController)
	{
		...
		return (selectedItems.Length() > 0); // we have a selection, and all selected items are link resources.
	}
	return false;
}
