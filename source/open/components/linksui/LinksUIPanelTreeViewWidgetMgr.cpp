//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/linksui/LinksUIPanelTreeViewWidgetMgr.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: gmatthew $
	...
	for(int i = 0; i < numHandlers; ++i) 
	{
		...
		if(iconID.fId == kInvalidRsrcID)
		{
			if(::GetClass(infoService) == kLinkInfoNameProviderBoss)
			{
				...
				thumbnailView->Resize(PMPoint(linksUIPrefs->GetCurrentRowHeight(),linksUIPrefs->GetCurrentRowHeight()));
				...
				if (expanderWidget)
				{
					expanderWidget->Resize(PMPoint(expanderWidget->GetFrame().Width(),linksUIPrefs->GetCurrentRowHeight()));
...
{	
	...
	for(int i = panelData->Length() - 1; i >= 0; --i) 
	{
		...
		if(::GetClass(linkInfoProv) == kLinkInfoNameProviderBoss) 
		{
			...
			nameView->Resize(PMPoint(nameWidth,kNewRowHeight));
...
//#endif
		...
		existingTextWidget->Resize(PMPoint(widgetWidth,kNewRowHeight));
		
		if(i == panelData->Length() - 1)
			newRowWidth = xLoc + widgetWidth;
	}
...
//	
	...
	{
		numDuplicates = linkEntry->GetChildEntries().Length();
	}
