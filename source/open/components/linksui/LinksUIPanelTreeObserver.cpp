//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/linksui/LinksUIPanelTreeObserver.cpp $
//  
//  Owner: lance bushore
//  
//  $Author: gmatthew $
...
//#endif				
	...
	if(IsMoreInfoSectionShowing()) // if more info section is not showing, it doesn't need to update.
	{
		...
		if(selectedItems.Length() == 1) // more info section only shows info when 1 item is selected, so don't bother updating otherwise.
		{
...
void LinksUIPanelTreeObserver::RebuildTree(bool attaching) const
{
	...
	if(!attaching)
	{
		...
		{
			TreeNodePtr<const LinksTreeUIDNodeID>	linksUIDNode(selectedItems[i]);
			const LinkTreeEntry* treeEntry = linksUIDNode->GetTreeEntry();
			if(treeEntry)
			{
...
void LinksUIPanelTreeObserver::HandleLinksUIPrefsChange(const LazyNotificationData* data)
{
	...
	{
		if(updatedItems[i] == kLinksUIPrefTreeStructureChangeMsgBoss)
		{
		...
		else if(updatedItems[i] == kLinksUITreeRowChangeMsgBoss)
		{
		...
		else if(updatedItems[i] == kLinksUIPrefInfoPaneChangeMsgBoss)
		{
...
void LinksUIPanelTreeObserver::UpdateLinkTreeHeaderWidget()
{
	...
	headerColumnData->SetHeaderIndent(0,kFalse,kFalse); // should match indent for tree expander widget.
