//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/dynamicdocumentsui/LiquidLayoutPanelObserver.cpp $
//  
//  Owner: Yeming Liu
//  
//  $Author: gmatthew $
...
{ 
	InterfacePtr<IPanelControlData> iPanelControlData(this, UseDefaultIID());	
	IControlView* iWidgetView = iPanelControlData->FindWidget(kLayoutAdjustmentWarningWID);
	InterfacePtr<ISubject> subject(iWidgetView, UseDefaultIID()); 
    if (subject) 
        subject->AttachObserver(this, IID_ITRISTATECONTROLDATA, IID_IAUTOLAYOUTWARNINGOBSERVER); 
	...
	if (context)
	{
		// Attach to Active context so I know when documents are closed or opened		
		InterfacePtr<ISubject> contextSubject( context, UseDefaultIID());
		if(contextSubject && !contextSubject->IsAttached(ISubject::kRegularAttachment, this, IID_IACTIVECONTEXT, IID_IAUTOLAYOUTWARNINGOBSERVER)  )
			contextSubject->AttachObserver(ISubject::kRegularAttachment, this, IID_IACTIVECONTEXT, IID_IAUTOLAYOUTWARNINGOBSERVER  );
		
		ActiveDocChange(context);
		UpdateLayoutAdjustmentWarning();
	}
} 
...
{ 
	...
	InterfacePtr<ISubject> subject(iWidgetView, UseDefaultIID()); 
    if (subject) 
        subject->DetachObserver(this, IID_ITRISTATECONTROLDATA, IID_IAUTOLAYOUTWARNINGOBSERVER); 

	InterfacePtr<ISubject> contextSubject( GetExecutionContextSession()->GetActiveContext(), UseDefaultIID());
	if(contextSubject && contextSubject->IsAttached(ISubject::kRegularAttachment, this, IID_IACTIVECONTEXT, IID_IAUTOLAYOUTWARNINGOBSERVER)  )
		contextSubject->DetachObserver(ISubject::kRegularAttachment, this, IID_IACTIVECONTEXT, IID_IAUTOLAYOUTWARNINGOBSERVER  );
...
{ 
	...
	else if (protocol == IID_ITRISTATECONTROLDATA)
	{
		...
		if ( widgetID == kLayoutAdjustmentWarningWID )
		{
			if (theChange.Get() == kTrueStateMessage)
			{
				// turn off layout adjustment
				SetAutoLayoutOff();
			}
		}
	}
...
void LayoutAdjustmentWarningObserver::LazyUpdate(ISubject* theSubject, const PMIID &protocol, const LazyNotificationData* )
{
	switch (protocol.Get())
	{
		case IID_IAUTOLAYOUTPREFERENCES:
			UpdateLayoutAdjustmentWarning();
			break;
		default:
			break;
...
void LayoutAdjustmentWarningObserver::AttachTo(IDocument* theDocument)
{
	if (theDocument)
	{
		InterfacePtr<IWorkspace> docWorkspace(theDocument->GetDocWorkSpace(), UseDefaultIID());
		InterfacePtr<ISubject> workspaceSubject(docWorkspace, UseDefaultIID());
		if ( workspaceSubject && !workspaceSubject->IsAttached(ISubject::kLazyAttachment,this, IID_IAUTOLAYOUTPREFERENCES, IID_IAUTOLAYOUTWARNINGOBSERVER) )
			workspaceSubject->AttachObserver(ISubject::kLazyAttachment,this, IID_IAUTOLAYOUTPREFERENCES, IID_IAUTOLAYOUTWARNINGOBSERVER);
	}
}
...
void LayoutAdjustmentWarningObserver::DetachFrom(IDocument* theDocument)
{
	if (theDocument)
	{
		...
		InterfacePtr<ISubject> workspaceSubject(docWorkspace, UseDefaultIID());
		if ( workspaceSubject )
			workspaceSubject->DetachObserver(ISubject::kLazyAttachment,this, IID_IAUTOLAYOUTPREFERENCES, IID_IAUTOLAYOUTWARNINGOBSERVER);
	}
}
...
{	
	InterfacePtr<IPanelControlData> iPanelControlData(this, UseDefaultIID());
	IControlView* layoutAdjustmentWarningView = iPanelControlData->FindWidget(kLayoutAdjustmentWarningWID);
	
	IActiveContext *ac = GetExecutionContextSession()->GetActiveContext();
	InterfacePtr<IAutoLayoutPrefs> iAutoLayoutPrefs((IAutoLayoutPrefs*)::QueryPreferences(IID_IAUTOLAYOUTPREFERENCES, ac));
	if ( iAutoLayoutPrefs )
	{
		layoutAdjustmentWarningView->ShowView(iAutoLayoutPrefs->GetAutoLayoutOn());
	}
}
...
{
	IActiveContext *ac = GetExecutionContextSession()->GetActiveContext();
	InterfacePtr<IAutoLayoutPrefs> theALPrefs((IAutoLayoutPrefs*)::QueryPreferences(IID_IAUTOLAYOUTPREFERENCES, ac));
	ASSERT_MSG(theALPrefs != nil, "QueryPreferences returned nil when getting IID_IAUTOLAYOUTPREFERENCES");
	if (!theALPrefs)
		return;
	
	bool16 bAutolayoutOn = theALPrefs->GetAutoLayoutOn();
	bool16 bStretchGraphics, bMoveGuides, bLoosenLocks, bStickyGuides, bIgnoreGuides;
	PMReal rAutoSnapZone;
	
	IDocument* doc = ac->GetContextDocument();
	if (bAutolayoutOn)
	{
		// turn it off but leave other parameters alone
		bStretchGraphics = theALPrefs->GetStretchGraphics();
		bMoveGuides = theALPrefs->GetMoveGuides();
		bLoosenLocks = theALPrefs->GetLoosenLocks();
		bStickyGuides = theALPrefs->GetStickyGuides();
		bIgnoreGuides = theALPrefs->GetIgnoreGuides();
		rAutoSnapZone = theALPrefs->GetAutoSnapZone();
		InterfacePtr<ICommand> command(CmdUtils::CreateCommand(kSetAutoLayoutPrefsCmdBoss));
		if (command)
		{
			InterfacePtr<IAutoLayoutPrefsCmdData> data(command, IID_IAUTOLAYOUTPREFSCMDDATA);
			data->Set(::GetUIDRef(doc), kFalse, bStretchGraphics, bMoveGuides, bLoosenLocks, bStickyGuides, bIgnoreGuides, rAutoSnapZone);
			CmdUtils::ProcessCommand(command);
		}
		else
		{
			ASSERT_FAIL("Failed to get autolayout command!");
		}
	}
}
