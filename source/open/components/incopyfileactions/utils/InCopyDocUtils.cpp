//========================================================================================
//  
//  $File: //depot/devtech/nevis/plugin/source/open/components/incopyfileactions/utils/InCopyDocUtils.cpp $
//  
//  Owner: Jon Pugh
//  
//  $Author: devtecbj $
...
private:
					...
					bool16		GetStoryForImport(IDocument* doc, UIDRef& storyToImport, IDFile* fileToOpen, IDataBase* db, UIDList* storyList, const PMString& formatType);
					...
					ErrorCode   HandleDocActionConflict(const UIDRef& doc, const ActionID& action, UIFlags uiFlags);
};
...
void InCopyDocUtils::DoClose(const UIDRef& doc, UIFlags uiFlags, bool16 allowCancel, IDocFileHandler::CloseCmdMode cmdMode)
{
	...
	if(HandleDocActionConflict(doc, kCloseActionID, uiFlags) != kSuccess)
	{
		// Cancel the action
		return;
	}
...
void InCopyDocUtils::ImportAllLinkedStories(IDocument* doc, const FileTypeInfoID& fileTypeID, IInCopyImportOptions* options)
{
	...
	for (int32 i=0; i < numPasses && globalErr != kCancel; ++i)
	{
		...
		// and links first.
		bool16 fileMissing = GetStoryForImport(doc, storyToImport, &fileToOpen, db, &storyList, formatTypeForPass);
		...
		while(storyToImport != UIDRef::gNull && globalErr != kCancel)
		{
			...
			else
			{
				...
				if (icwgHelper->ShouldSyncPrePlace(storyToImport)) 
				{
					...
					{
						fileMissing = GetStoryForImport(doc, storyToImport, &fileToOpen, db, &storyList, formatTypeForPass);
...
//-----
// GetStoryForImport
bool16 InCopyDocUtils::GetStoryForImport(IDocument* doc, UIDRef& storyToImport, IDFile* fileToOpen, IDataBase* db, UIDList *importedStories, const PMString& formatType)
{
...
ErrorCode InCopyDocUtils::HandleDocActionConflict(const UIDRef& doc, const ActionID& action, UIFlags uiFlags)
{
	...
	if(bSavingInBG || Utils<IBGTasksUtils>()->IsDocumentInUseByBGTasks(doc))
	{
